...
export const tripExpenseRelations = relations(tripExpenses, ({ one }) => ({
  trip: one(trips, {
    fields: [tripExpenses.tripId],
    references: [trips.id],
  }),
  category: one(spendCategories, {
    fields: [tripExpenses.categoryId],
    references: [spendCategories.id],
  }),
}));
...
export type TripExpenseSelect = typeof tripExpenses.$inferSelect;
export type TripExpenseInsert = typeof tripExpenses.$inferInsert;